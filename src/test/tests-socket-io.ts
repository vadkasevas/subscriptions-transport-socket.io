...
import { GRAPHQL_SUBSCRIPTIONS } from '../protocol';

import { createServer, IncomingMessage, ServerResponse, Server } from 'http';
import { SubscriptionServer, ExecutionParams } from '../server';
import { SubscriptionClient } from '../client-io';
    ...
    fields: {
      ...
      userFiltered: {
        ...
        subscribe: withFilter(
          () => testPubsub.asyncIterator('userFiltered'),
...
const handlers = {
  onOperation: (
    ...
    webSocketRequest: io.Socket,
  ) => {
    return Promise.resolve(
      Object.assign({}, params, { context: msg.payload.context }),
    );
  },
};
...
function notFoundRequestListener(
...
new SubscriptionServer(
  Object.assign({}, options, { keepAlive: 500 }),
...
const eventsServer = new SubscriptionServer(
  eventsOptions,
...
describe('Client', function() {
  ...
  beforeEach(() => {
    wsServer = io(options).listen(RAW_TEST_PORT);
  });
  ...
  //   const subscriptionsClient = new SubscriptionClient(`http://localhost:${TEST_PORT}/`);
  ...
  it('should send connectionParams along with init message', done => {
    ...
    wsServer.on('connection', (connection: any) => {
      connection.on('message', (message: any) => {
        const parsedMessage = JSON.parse(message);
        expect(JSON.stringify(parsedMessage.payload)).to.equal(
  ...
  //           setTimeout(() => {
  //             expect(client.status).to.equals('Lul');
  //             done();
  //           }, 500);
  //         },
  //       );
  ...
  //     connection.on('message', (message: any) => {
  //       connection.send(
  //         JSON.stringify({ type: MessageTypes.GQL_CONNECTION_ACK }),
  //       );
  //     });
  //   });
  ...
  //   const client: SubscriptionClient = new SubscriptionClient(`http://localhost:${RAW_TEST_PORT}/`, {
  //     lazy: true,
  //   });
  ...
  //     }
  //   });
  //   client = new SubscriptionClient(`http://localhost:${RAW_TEST_PORT}/`, { reconnect: true });
  ...
  //   client = new SubscriptionClient(`http://localhost:${TEST_PORT}/`, { reconnect: true });
  ...
  //   const subscriptionsClient = new SubscriptionClient(`http://localhost:${RAW_TEST_PORT}/`, {
  //     timeout: 500,
  //     reconnect: true,
  ...
  //   const subscriptionsClient = new SubscriptionClient(`http://localhost:${KEEP_ALIVE_TEST_PORT}/`, { timeout: 600 });
...
//     expect(() => {
//       new SubscriptionServer({ execute: undefined }, { server: httpServer }, 'io');
//     }).to.throw();
//   });

//   it('should throw an exception when creating a server with subscribe only', () => {
//     expect(() => {
//       new SubscriptionServer({ subscribe: {} as any }, { server: httpServer }, 'io');
//     }).to.throw();
//   });
...
//     expect(() => {
//       new SubscriptionServer({ execute: {} as any }, { server: httpServer }, 'io');
//     }).to.throw();
//   });
...
//     const client = new SubscriptionClient(`http://localhost:${SERVER_EXECUTOR_TESTS_PORT}/`);
//     client.onConnected(() => {
//       client.request({
...
//     const subscriptionsClient = new SubscriptionClient(`http://localhost:${ONCONNECT_ERROR_TEST_PORT}/`, {
...
//     const client = new SubscriptionClient(`http://localhost:${EVENTS_TEST_PORT}/`);
//     setTimeout(() => {
//       client.client.close();
//     }, 100);
//     setTimeout(() => {
...
//     const client = new WebSocket(`http://localhost:${TEST_PORT}/`);
...
//     const client = new WebSocket(`http://localhost:${TEST_PORT}/`, GRAPHQL_SUBSCRIPTIONS);
...
//     let client = new WebSocket(`http://localhost:${KEEP_ALIVE_TEST_PORT}/`, GRAPHQL_SUBSCRIPTIONS);
//     let yieldCount = 0;
//     client.onmessage = (message: any) => {
